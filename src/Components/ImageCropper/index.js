...
function CropperModal(props) {
    ...
    return (
        <div 
            ...
            onMouseMove={(e) => dispatch({ type: 'DRAGGING', clientX: e.clientX, clientY: e.clientY})}
            ...
            <div style={{width: state.width, height: state.height}}>
                ...
                <div className={classes.buttonBar}>
                        <Fab size="small" component="label" 
                            ...
                            onMouseUp={() => dispatch({ type: 'ZOOM_END'})}><ZoomIn/></Fab>
                        <Fab size="small" component="label" 
                            onMouseDown={() => {
                                dispatch({ type: "ZOOM_START" });
                                handleZoom("ZOOM_OUT");
                            }}  
                            onMouseUp={() => dispatch({ type: 'ZOOM_END'})}><ZoomOut/></Fab>

                        <Fab size="small" color="secondary" component="label" onClick={handleCrop}><Check/></Fab>
                        <Fab size="small" className={classes.close} component="label" onClick={() => dispatch({ type: 'CLOSE'})}><Close/></Fab>
                </div>
            </div>
        </div>
    )
...
function Cropper(props) {
    ...
    return (
        <div className={classes.root}>
            ...
            <input id={fileId} type="file" accept="image/*" className={classes.hidden} onChange={handleUpload}/>
            <img src={image} alt={altText}/>
            {state.open ? <CropperModal {...{image, state, dispatch, classes, onCropComplete}}/> : null}
        </div>
    )
}

function ImageCropper(props) {
    const {editable, image, altText, width, height, classes, onCropComplete} = props;
    ...
    return <div className={classes.root}><img src={image} alt={altText}/></div>;
}
