...
function Step(props) {
    ...
    function updateStep(value, index) {
        let newSteps = steps;
    ...
    if (edit) {
        return (
            <div className={classes.stepRow}>
                ...
                />
                <Fab aria-label="Delete" size="small" className={classes.delete} onClick={() => removeStep(index)}>
                    <Delete />
                </Fab>
            </div>
        );
...
function Steps(props) {
    ...
    let controls;
    if (edit) {
        controls = <Button variant="contained" color="secondary" onClick={addStep}>Add step</Button>;
    }
    return (
        <>
